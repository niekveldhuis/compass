...
To open the scripts, open the Anaconda Prompt (Windows) that was installed with Anaconda or the Terminal (OS X) and type `jupyter notebook`. This will open a local web page with a listing of the current directory. Navigate to the place where the [Compass][compass] files are located and open a notebook (extension `.ipynb`) by clicking on it.
