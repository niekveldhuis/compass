...
#    `equivalencies.json`: a set of equivalence dictionaries used at various places in the parser. The directory and the file are included in the [COMPASS](https://github.com/niekveldhuis/compass) file set (see section 1.4.1).
# 3. Directory `output`.
...
# 
# The `equivalencies.json` file is loaded with the `json` library (for JSON and the `json` library, see section 2.1.1). The dictionaries `suxwords`, `emesalwords` and `propernouns` (which, together, contain the entire [ETCSL](http://etcsl.orinst.ox.ac.uk) vocabulary) are concatenated into a single dictionary.
...
# :::
# 
# The replacement code uses the re.sub() function with a temporary lambda function to translate the regular digits into their Unicode subscript counterparts. Using a function in re.sub() was discussed above in section 2.2.3:
# 
# ```python
...
# 
# The function `getword()`, which is called by the function `getline()`,  is the most complex of the series of functions that parses the [ETCSL](http://etcsl.orinst.ox.ac.uk). This is the case because different types of words (regular Sumerian words, proper nouns, Emesal words, Akkadian glosses) are processed in different ways.
# 
...
# :::{note}
# 
# A regular Sumerian lemma ([ŋen](http://oracc.org/epsd2/o0029256) = "to go") in [ETCSL](https://etcsl.orinst.ox.ac.uk/) XML:
# 
# ```XML
...
# 
# :::{note}
# The Emesal form of the Sumerian lemma [inim](http://oracc.org/epsd2/o0031116) ("word") is spelled e-ne-eŋ₃, which looks as follows in the [ETCSL](https://etcsl.orinst.ox.ac.uk/) XML:
# 
# ```xml
...
# 
# :::{note}
# Example of a proper noun (the temple name Eana) in [ETCSL](https://etcsl.orinst.ox.ac.uk/) XML:
# 
# ```xml
...
# ```
# If getline() receives a gap node it copies all the meta data in the dictionary `meta_d` into the dictionary `line` and adds a field `extent` (the length of the gap). This data is found in the attribute `extent` of the gap node. This dictionary is then appended to the list `alltexts` and control is returned to the function `getsection()`. Each row in `alltexts`, therefore, will represent either a word or a gap.
# 
...
# 
# The function `getversion()` is called by the function `parsetext()` and receives two arguments: `tree` (the `etree` object) and `meta_d` (the dictionary of meta data). In the XML tree versions are marked by a node `body` with a child `head`. The node `head` contains the name of the version. Iterating through the versions, the function updates the key "version" in `meta_d` with the name of the version and then calls the `getsection()` function. The first argument is the portion of the tree that represents the version that is being parsed, the second argument is `meta_d`. If a composition is not divided into versions the entire tree is passed to `getsection()` and the version name in `meta_d` is the empty string.
# 
# In some cases version names are very long and somewhat unwieldy. The equivalency list `versions` (in `equivalencies/equivalencies.json`; see 2.2.2.2) is used to adjust version names.
...
# 
# The main process also defines a number of variables (compiled regular expressions and translation tables) that are used later on in the process for adjusting transliteration conventions in the function `tounicode()` (see [2.2.5](#2.2.5-Transliteration-Conventions)
# 
# After the loop has run its course (this may take a few minutes) the list `alltexts` is transformed into a `pandas` DataFrame. The progress bar should indicate that 394 files have been processed and the resulting dataframe should have 12 columns and 170,856 rows, each row representing a word or a gap. All missing values (`NaN`) are replaced by empty strings. 
...
# :::{margin}
# 
# For manipulating a dataframe in `pandas`, see Chapter 2.1: Data Acquisition ORACC (see the [Basic ORACC-JSON Parser](../2_1_Data_Acquisition_ORACC/2_1_3_basic_ORACC-JSON_parser.ipynb)).
# 
# :::
