...
The search engine uses "widgets" (pieces of software to create a user interface) to call the search function and display the results. It creates a text box (for the search term), a slider (for the maximum number of hits), a checkbox and a button, to be clicked to run the search. A drop-down menu provides the possibility to order search results in a variety of ways (note that ordering by date is done alphabetically - Amar-Sîn dates will always come first). On a fast machine one may do away with the button and have the software react immediately to user input (see Alternative Interface in the notebook).
