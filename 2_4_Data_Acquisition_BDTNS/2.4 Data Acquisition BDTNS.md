...
When searching for sar-ki we should not find sar kin, in other words, the search should identify only full sign names. Sign names are separated from each other by spaces. By adding a space before and after each line (in the `sign_name` column) each sign and each sign sequence is preced and followed by a space, evenm when it appears at the beginning or the end of the line. The (transformed) search input is also put between spaces.
