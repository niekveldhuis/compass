...
"""The lexicalrichness module was written by Lucas Shen (https://pypi.org/project/lexicalrichness/) and adapted by Niek Veldhuis (2019) for the Compass project (http://github.com/compass). The adaptations allow to input a preprocessed and tokenized text. The option to use TextBlob for tokenization has been removed.

Usage: LexicalRichness(list) # argument is a preprocessed list of tokens.
or: LexicalRichness(text, use_dummy=False) # first argument is a string which will be preprocessed and tokenized.
"""
...
class LexicalRichness(object):
    ...
    def __init__(self, text, use_tokenizer= False):
        """ Initialise object with basic attributes needed to compute the common lexical diversity measures.
            ...
            ----------
            text: string or list
                String (or unicode) variable containing textual data; set use_dummy to False
            use_dummy: bool
                If True, do not tokenize or preprocess.
            ...
            tokenize(string)
                Tokenize text using built-in string methods (if use_dummy=False)
        """
